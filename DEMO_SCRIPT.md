...
- The realtime call is **brokered** — the API key never touches the
  ...
  hard cap.
- The full feature stack at v4 lives at `hackathon-submission-v4`.
  For the character-driven flow, see Path F below
  (`hackathon-submission-v5`).
...
**`demo ready · concepts mocked`** (emerald) with `Image Gen
(Runway): real`, `Video Gen (Runway): real`, and `cap: 200,000` chips
visible. Total real spend: ~5 credits for the character
(`gen4_image_turbo` + `/v1/avatars`) plus the regular ad-pipeline
credits — well under 0.05% of the 50k pool.

### Pre-flight (off camera)

1. `RUNWAY_API_KEY` set in repo-root `.env`. `OPENAI_API_KEY` blank
   for the deterministic mock concept story.
2. **Pre-warm the mic permission prompt.** Visit
   `http://localhost:5173`, find a saved card with a READY avatar
   (or the Brewster character avatar from a prior dry-run), click
   *Start Conversation* once → Allow mic → End Conversation.
3. **Pre-create the character off camera** *(strongly recommended
   for live recording)*. Open Character Studio, create
   `Brewster the Bear` with template `mascot`, voice `max`, subject
   `a friendly grizzly bear barista mascot, warm hospitable smile,
   coffee shop apron`. Wait for portrait generation (~14 s) and
   `Create Runway Avatar` (~30–45 s) to complete so the recording
   doesn't include the wait. The character persists in
   `backend/data/characters.json` between sessions.
4. Optional clean slate of stale campaigns:
   `echo '[]' > backend/data/campaigns.json`. The Character Studio
   library is independent of campaigns and survives this.
...
|---|---|---|
| 0:00 | Page load | Mode banner emerald readiness chip + cap chip |
| 0:08 | Narrate: "AdSpark is no longer just an AI ad generator. It's a Character Studio — generate a reusable brand mascot, then drop them into every campaign you build." | Mode banner |
| 0:18 | Scroll to **Character Studio** panel above the gallery | Pink Runway-powered pill on the section header; existing characters in the library grid |
| 0:25 | Hover over Brewster's tile (pre-created); narrate: "We made Brewster earlier — a brand mascot for our cold-brew company. Stylised bear, warm hospitable smile, coffee apron. Generated by Runway from a template prompt, then bound to a Runway Avatar." | Brewster's portrait + status pill (`avatar ready`) |
| 0:40 | Click **+ Create Character** to show the form briefly *(optional, off recommended path)* | Inline form with name / template / voice / personality fields |
| 0:50 | Cancel out of the form. Narrate: "Same thing for any business — pick a template (mascot, founder, coach, local guide), give it a name and a voice, and Runway generates the portrait + binds the avatar in about a minute." | Form collapses; library grid visible again |
| 1:00 | Form: `Donkey Betz Coffee` / `Reels-ready cold brew` / `warm cinematic` / `morning commuters` | Form fills |
| 1:15 | **Generate Ad Concepts** → pick `Daily Ritual` | Three concept cards, recommended pill |
| 1:25 | Settings — Model `Gen-4.5`, Source ratio `Reels / TikTok 720:1280`, Duration `5 s` | Settings chip row updates |
| 1:35 | Click **Generate Reference Image** | ~25–45 s — narrate that this is the campaign hero shot, separate from the character |
| ~2:15 | Image renders inline; click **Generate Video** | Gen-4.5 5 s — typically 2–3 min in real mode |
| ~4:15 | `SUCCEEDED` → click **Save campaign card** | Cache pill `cached locally` |
| ~4:20 | Scroll to Brand Spokesperson section. Narrate: "Now we attach Brewster." | The campaign's Brand Spokesperson section shows the existing avatar picker AND a new `Attach Character` affordance |
| ~4:25 | Click **Attach Character** → click `Use Brewster` in the inline picker | Pink-bordered `Attached Character: Brewster the Bear` block appears with portrait + name + template/voice/status; Avatar Picker + Create Custom flow hide |
| ~4:30 | Click **Present Campaign** under Avatar Host Clip | Status pill PENDING → RUNNING → ~10 s → host clip plays |
| ~4:45 | Narrate: "That's Brewster — the same character we generated up top — narrating this campaign." | Inline `<video>` plays Brewster speaking the campaign pitch |
| 5:00 | Click **Design Brand Voice** | ~10 s; preview MP3 plays inline |
| 5:15 | Click **Spanish** dub button | ~25 s; Spanish dub plays |
| 5:45 | Click **Start Conversation** under Talk to Brand Spokesperson | "Connecting…" → live avatar feed appears with Brewster's processed thumbnail; "live · 4:59" countdown |
| 5:55 | Speak: *"Pitch Donkey Betz Coffee in one sentence."* | Brewster responds in `max` voice |
| 6:30 | Click **End Conversation** | Section returns to idle |
| 6:40 | Narrate the closing: "One brief, one character, one Runway-powered identity — driving the ad, the host clip, the brand voice, the multilingual dubs, and a live conversation. Brewster is the durable IP. The campaign is just one of many things the brand can build with him." | Closing summary |

### Pass criteria (run silently mid-recording)

- Mode banner emerald.
- Character Studio panel renders with pink Runway-powered pill.
- Brewster's status pill reads `avatar ready` before the demo starts.
- After Attach Character, the campaign shows the pink Attached
  Character block; Avatar Picker + Create Custom flows are hidden.
- Avatar Host Clip uses Brewster's avatar id (visible in any
  inline avatar id label or via DevTools → Network → host-video
  response body).
- Realtime: Brewster's processed thumbnail appears in the avatar
  feed; audio round-trips both directions; countdown ticks; End
  Conversation cleanly returns to idle.
- Audio Pack: voice preview + at least one dub plays.
- No browser console errors mentioning `rwk_`, `RUNWAY_API_KEY`,
  `Bearer rwk_`, or `sessionKey` (all should stay server-side).
- `backend/data/characters/<brewster-id>-portrait.png` exists on
  disk; `backend/data/host/<campaign-id>.mp4` and
  `backend/data/audio/<campaign-id>-*.mp3` exist after recording.

### Optional B-roll: raccoon coffee mascot

If you want a second character on screen (e.g. to show a
brand-portfolio story for a multi-business operator), pre-create
`Rosie the Raccoon` off camera:

- Template: `mascot`
- Voice: `aurora` (or any of the 30 universal presets)
- Subject: `a friendly raccoon barista mascot in apron, holding a
  cold brew, warm shop lighting`
- Style: `photorealistic stylised plush texture`

The library grid will show both Brewster and Rosie. On camera,
narrate: "We can keep adding characters — different mascots for
different product lines, founder portraits for different brands."
Don't attach Rosie mid-recording — she's a visual cue only, since
the demo follows Brewster end-to-end.

### Fallback if the character avatar isn't ready mid-recording

Phase K's `create-avatar` step takes ~30–45 s of `/v1/avatars`
processing time. If you skipped pre-flight and the character status
is still `pending` when you reach the Attach step:

1. Click **Use Mock Avatar** *(if the campaign has a `host_avatar_id`
   already)* or fall back to the **Avatar Picker** for an existing
   ready avatar — Path F still works with the resolution chain
   (selected wins over host).
2. Or switch to mock mode mid-recording (Path C technique). The
   character's mock avatar binds instantly with the cached stdlib
   portrait as the thumbnail. The realtime button cleanly disables
   in mock mode but the host clip still plays via the ffmpeg lavfi
   placeholder.

### Anti-patterns specific to Path F

- **Don't generate a brand-new character on camera unless you
  budgeted for a 14 + 35 s wait.** Pre-create off camera.
- **Don't claim characters are "marketed" or "exported" today.**
  V1 is reuse-within-this-account; marketplace + export/import is
  Phase M roadmap.
- **Don't show character portraits that fail Runway's avatar
  processor.** The four locked templates (mascot / founder / coach
  / local_guide) are tuned to consistently produce front-facing
  faces. Custom subjects can drift cartoony or off-center —
  pre-screen any new character before recording.
- **Don't claim character voice is editable post-creation.**
  Runway doesn't expose PATCH for avatar voice. Changing the
  character's voice today requires re-running `Create Runway
  Avatar`, which mints a fresh avatar resource on the account.
  K.5 `replace-avatar` is roadmap.
- **Don't mention character DELETE bypassing the Runway side.**
  Local delete cleans the portrait + JSON entry; the Runway-side
  avatar persists. That's intentional V1 behavior — the user can
  still use the avatar from the Avatar Picker even after deleting
  the local Character record.

### What Path F demonstrates

- **Characters are first-class.** They live in their own resource
  (`/api/characters`, `backend/data/characters.json`) outside any
  single campaign. One character can drive any number of campaigns.
- **The avatar resolution chain works.** Attaching Brewster
...
- **Mock-mode parity is preserved.** The same UI works without keys
  — stdlib PNG portrait, synthetic avatar id, ffmpeg-lavfi host
  clip. The character library survives between mock and real
  sessions on the same machine.
- **The full v5 feature stack lives at `hackathon-submission-v5`.**
  `git checkout hackathon-submission-v5` reproduces this exact
  flow.
