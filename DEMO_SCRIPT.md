...
3. **Pre-create Brewster off camera** *(still strongly recommended
   for live recording)*. Stage 1 = Spokesperson now lives at the top
   of the page, but generating Brewster's portrait + binding the
   Runway Avatar still takes ~50 s of real wall-clock — too slow for
   the recording. Open Stage 1 → click `+ Create Character` →
   `Brewster the Bear`, template `mascot`, voice `max`, subject
   `a friendly grizzly bear barista mascot, warm hospitable smile,
   coffee shop apron`. Wait for portrait generation + `Create Runway
   Avatar` to complete. The character persists in
   `backend/data/characters.json` between sessions; the spokesperson
   selection persists in localStorage under
   `adspark.spokesperson.v1`.
4. **Click "Use as Spokesperson" on Brewster's tile** so he's the
   active spokesperson. The orange "active" pill should be visible
   on his tile.
5. Optional clean slate of stale campaigns:
   `echo '[]' > backend/data/campaigns.json`. Character Studio +
   active spokesperson are independent of campaigns and survive this.
...
| 0:55 | Stage 3 unfolds. Notice the Visual Source banner: "Using Brewster the Bear as the visual source from Stage 1." | Spark-tinted banner above the radio tiles; the `Use Character` radio is preselected; Brewster's portrait is the picked character |
| 1:05 | Settings — Model `Gen-4.5`, Source ratio `Reels / TikTok 720:1280`, Duration `5 s` | Settings chip row updates |
| 1:15 | Generate Video button reads "Generate Video with Brewster the Bear". Click it. | Gen-4.5 5 s with Brewster's portrait as the input — typically 2–3 min in real mode |
| ~3:15 | `SUCCEEDED` → click **Save campaign card** | Cache pill `cached locally`. Because Brewster was the active spokesperson, the new gallery card opens with the Character tab already populated. No manual attach needed. |
| ~3:25 | Scroll to the saved card's Character tab. Narrate: "Brewster came along automatically — the active spokesperson auto-attaches on save." | Pink Attached Character block with Brewster's portrait |
| ~3:35 | Click **Present Campaign** under Avatar Host Clip | Status pill PENDING → RUNNING → ~10 s → host clip plays |
| ~3:50 | Narrate: "Brewster narrating the ad. Same character we picked at Stage 1." | Inline `<video>` plays Brewster speaking the campaign pitch |
| 4:00 | Switch to Visuals tab. Click **Build Commercial with Voice**. | Spark-tinted section; ~3 s ffmpeg pass; voiced commercial MP4 plays inline (visual + Brewster's voice) |
| 4:10 | Switch to Voice tab. Click **Design Brand Voice** | ~10 s; preview MP3 plays inline |
| 4:25 | Click **Sample Spanish** | ~25 s; Spanish voice sample plays |
