...
// PR CA — Legacy app shell. Renders the v1 four-stage wizard
// (PR A through PR AZ-era stack) verbatim. The v2
// SpokespersonStudio / lanes / mode modal moved to the new
// `<Library>` route at `/`. uxFlag.js is no longer consulted
// here — this component is the "always v1" surface, reachable
// only via /legacy. Do not re-introduce isUxV2() conditionals
// here; they belong on the parent shell only (the one-shot
// v1 redirect lives in AppShell).
...
export default function LegacyApp() {
  ...
  // PR BS — companion target tab. PR CA: kept null on /legacy
  // since SpokespersonStudio (the click-through source) no
  // longer renders here; CampaignGallery still accepts the
  // prop so deep-linked v2 jumps from the future workspace
  // (PR CB+) can drive it.
    ...
    <div className="min-h-full">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <Stage
          ...
          accent="pink"
        >
          {/* PR CA — /legacy is the always-v1 surface. The v2
              SpokespersonStudio / lanes / mode modal moved to the
              new `<Library>` route at `/`. CharacterStudio is the
              only component mounted here; the v2 click-through
              state (openCampaignId / openCampaignTab) still feeds
              CampaignGallery so the "Open in gallery →" affordance
              continues to work for any visitor who lands directly
              on /legacy through the top-bar Legacy link. */}
          <CharacterStudio
            onCharactersChanged={() => {
        ...
        <footer className="text-xs text-zinc-600 pt-6 border-t border-zinc-900/60 flex items-center justify-between gap-2 flex-wrap">
          ...
          {/* PR CA — Footer "Try preview UX / Use classic UX" toggle
              ...
              <AppShell> outlet is the canonical round-trip between
              `/` (Spokesperson Library) and `/legacy` (this
              wizard). uxFlag.js stays in the codebase as a one-shot
              localStorage migration helper inside AppShell. */}
        </footer>
      </div>
    </div>
  )
