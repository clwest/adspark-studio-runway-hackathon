...
context-kit refresh after PR CG (Demo Spokespeople Fixtures)
on top of PR AG–CF / SESSION 011 anchors. Backend route count
remains **70** (PR CG is fixture-only — no code or routes
changed). Library now seats **7 spokespeople by default**:
the four new demo fixtures (Brewster Bolt · mascot · drew;
Clara Vale · founder · clara; Rex Roadside · local_guide ·
marcus; Mina Spark · coach · ruby — each tagged
`metadata.demo=true`) plus the three pre-existing user
records (Brewster the Raccoon, Piper Voltage, Sir
Landsloplot) which were not touched. Idempotent seeder lives
at `scripts/seed-demo-spokespeople.py`. Lane scaffold
