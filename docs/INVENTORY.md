...
context-kit refresh after PR BG (Spokesperson Appearances Tab
Wiring) on top of the PR AG–BF / SESSION 011 anchors. Backend
route count is **68** application routes — PR BG is the fourth
gated v2 slice from the spokesperson-first redesign (after BD's
flag + helpers, BE's scaffold, BF's Knowledge tab). The
Appearances tab on each SpokespersonCard now lists every linked
campaign with an inferred mode badge (Cinematic / Spokesperson
Ad / Dialogue Scene / Storyboard / Realtime / Mixed / Draft),
last-touched relative time, and a compact output-summary row.
With BG, the v2 Spokesperson card's first pass is complete:
Identity + Knowledge + Appearances all show real data. Default
