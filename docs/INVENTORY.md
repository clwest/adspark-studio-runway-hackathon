...
context-kit refresh after PR CC (Spokesperson Workspace
Shell + Identity Tab) on top of the PR AG–CB / SESSION 011
anchors. Backend route count remains **70** (PR CC is
frontend-only — no new endpoints; reuses every existing
`/api/characters/*` route the legacy CharacterStudio uses).
The frontend now has three top-level routes: `/` mounts the
**Spokesperson Library** (PR CA + PR CB), `/spokespeople/:id`
mounts the new dedicated **Spokesperson Workspace** (PR CC,
header + 5-tab nav: Identity | Knowledge | Campaigns |
Conversations | Outputs), and `/legacy` mounts the verbatim
v1 four-stage wizard + saved-campaign gallery (frozen,
demo-fallback). For PR CC the **Identity tab is fully
implemented** (embeds the existing CharacterCard with every
voice + portrait + avatar + drift handler wired); the other
four tabs render compact "lands next" placeholders. The
workspace's `+ New Campaign` button opens the existing
`<CampaignModeModal>`, persists mode + active spokesperson
to localStorage, and navigates back to `/` where the
Library lane already mounts the right surface for the right
spokesperson on first paint.
