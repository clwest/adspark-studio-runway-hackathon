...
Snapshot of what is real, mocked, and key-dependent as of PR L
(submission docs refresh for `hackathon-submission-v5`).
...
| `app/routers/campaigns.py` | real | All `/api/campaigns/*` routes (17 of them now, see endpoint table below — adds `attach-character`) |
...
POST   /api/characters/{id}/generate-portrait              (PR K — gen4_image_turbo)
