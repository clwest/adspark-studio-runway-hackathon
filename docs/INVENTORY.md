...
context-kit refresh after PR CA (App Shell + Router +
Spokesperson Library as Home) on top of the PR AG–BU /
SESSION 011 anchors. Backend route count remains **70**
(PR CA is a frontend-only architecture refactor — no new
backend routes). The frontend is now organised around two
top-level routes: `/` mounts the Spokesperson Library
(default homepage; the v2 surface graduated to default
in PR CA, no flag required), and `/legacy` mounts the
verbatim v1 four-stage wizard + saved-campaign gallery
(frozen, demo-fallback). A persistent `<TopBar>` carries
the AdSpark logo, a tiny health pill (mock vs live), and
a `Legacy UI ↗` link. The footer "Try preview UX / Use
classic UX" toggle is gone.
