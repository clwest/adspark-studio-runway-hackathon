...
context-kit refresh after PR AQ (Avatar PATCH for Custom Voice
Swap) on top of the PR AG–AP / SESSION 011 anchors. Backend
route count is **66** application routes — PR AQ adds one new
endpoint (`POST /api/characters/{id}/apply-voice`) on top of the
65 routes from PR AP and runs an automatic PATCH on the existing
avatar from inside the PR AN `/clone-voice` route.
