...
context-kit refresh after PR CD (Home Library Only) on top
of the PR AG–CC / SESSION 011 anchors. Backend route count
remains **70** (PR CD is frontend-only). The frontend
preserves the three-route shape established by PR CA: `/`
mounts the **Spokesperson Library**, `/spokespeople/:id`
mounts the **Spokesperson Workspace** (PR CC), and `/legacy`
mounts the verbatim v1 four-stage wizard + saved-campaign
gallery. PR CD strips the campaign-creation surfaces off the
homepage entirely — the global "+ New Campaign" button, the
selected-mode pill, the Spokesperson / Cinematic / Dialogue
lane components, and the CampaignModeModal **only mount
inside `/spokespeople/:id` now**. The Library passes a new
`hideCampaignControls` prop to `<SpokespersonStudio>`; the
workspace keeps its own "+ New Campaign" CTA but the mode
modal hands off to `/legacy` (the verbatim wizard remains
the canonical campaign-creation surface until the upcoming
slice mounts lanes inside the workspace's Campaigns tab).
