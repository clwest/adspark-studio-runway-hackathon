...
PR CH Demo Campaign Fixtures for New Spokespeople).
...
- **`/`** — Spokesperson Library. The default homepage. After
  PR CD this is **library only**: a Heading + tagline + 4-chip
  stats row + primary `+ Create Spokesperson` CTA + tiles +
  per-tile `Open Spokesperson →` link. The campaign-creation
  surfaces (`+ New Campaign`, mode pill, lane components,
  `<CampaignModeModal>`) are suppressed via the new
  `hideCampaignControls` prop on `<SpokespersonStudio>`.
  Sections 1–11 below describe the working surfaces; for v2
  they now live inside `/spokespeople/:id` (workspace) or
  `/legacy` (the wizard's legacy fallback).
